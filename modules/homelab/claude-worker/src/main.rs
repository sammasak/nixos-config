...
struct DeployedUrlResponse {
    url: String,
}

#[derive(Deserialize)]
struct SetDeployedUrl {
    url: String,
}
...
async fn main() {
    ...
    let deployed_url_file = base.join("workspace").join(".deployed-url");
    ...
    let app = Router::new()
        ...
        .route("/deployed-url", get(get_deployed_url).post(set_deployed_url))
        .with_state(state);
...
async fn get_deployed_url(
    State(state): State<Arc<AppState>>,
) -> Result<Json<DeployedUrlResponse>, StatusCode> {
    match fs::read_to_string(&state.deployed_url_file).await {
        Ok(content) => {
            let url = content.trim().to_string();
            if url.is_empty() {
                Err(StatusCode::NOT_FOUND)
            } else {
                Ok(Json(DeployedUrlResponse { url }))
            }
        }
...
async fn set_deployed_url(
    State(state): State<Arc<AppState>>,
    Json(payload): Json<SetDeployedUrl>,
) -> StatusCode {
    match fs::write(&state.deployed_url_file, payload.url.trim()).await {
        Ok(_) => StatusCode::NO_CONTENT,
        Err(e) => {
            eprintln!("Failed to write deployed-url: {}", e);
